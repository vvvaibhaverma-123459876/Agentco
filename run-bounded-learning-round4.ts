...
async function runRound4() {
  ...
  // Use arxiv source pack that was working in previous rounds
  // This ensures we fetch from a source discovery registry that's been validated
  const config = {
    goal: 'Round 4: Extract high-quality, diverse AI research claims from verified arXiv sources',
    sourcePack: 'arxiv_ml_research', // Source discovery registry
    maxPages: 15, // Fetch up to 15 different papers
    maxClaims: 40, // Extract up to 40 claims
    maxDurationSeconds: 300,
    provider: 'openai' as const,
    realWebEnabled: true, // Actually fetch from arXiv
    dryRun: false,
