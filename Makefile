...
no-blocking-findings:
	@command -v $(PYTHON) >/dev/null || (echo "Python 3.13 is required. Install the configured interpreter or run with PYTHON=/path/to/interpreter"; exit 1)
	@$(PYTHON) scripts/verify_no_blocking_findings.py --check

subsystem-audit-results:
	@command -v $(PYTHON) >/dev/null || (echo "Python 3.13 is required. Install the configured interpreter or run with PYTHON=/path/to/interpreter"; exit 1)
	@$(PYTHON) scripts/run_subsystem_audit_results.py

subsystem-audit-results-check:
	@command -v $(PYTHON) >/dev/null || (echo "Python 3.13 is required. Install the configured interpreter or run with PYTHON=/path/to/interpreter"; exit 1)
	@$(PYTHON) scripts/verify_subsystem_audit_results.py --check
...
release-gate:
	...
	@echo "== [0c/12] blocking findings check =="
	@$(MAKE) no-blocking-findings
	@echo "== [0d/12] subsystem audit results check =="
	@$(MAKE) subsystem-audit-results-check
