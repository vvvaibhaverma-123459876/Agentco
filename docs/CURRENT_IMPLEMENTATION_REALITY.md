...
| Institution claim vetting flow | Passed DB-backed Production -> Verification -> Adversarial -> Audit claim-vetting service with lite/full rigor tiers and auditable adversarial reputation-adjustment requirements |
| Institutional synthesis role | Passed DB-backed synthesis over completed findings with dependency-graph provenance, weakest-link confidence bounding, independent-source lift, conservative DERIVED/CONJECTURE typing, and Verification/Audit event checks |
...
- Autonomy specialist delegation reaches the 17 registered `agents/autonomy/*` roles through `TeamActivationService` spawning `python3.13 -m agents.autonomy.<role>` by default. Department-style V1/V2 agent classes elsewhere under `agents/` are not activated by the `spawn_specialist` action and should be treated as separate inventory unless another caller wires them in.
...
- Institution claim vetting now has a civilization-native department pipeline: Production records the draft, Verification enforces grounding/type checks, full rigor requires a substantive Adversarial break attempt, Audit records provenance through `work_cycle_events`, and the work request stores the resulting finding/rejection summary. Adversarial failures record required reputation adjustments without bypassing the existing protected reputation propagation guard.
- Institutional synthesis composes only completed source findings, records the dependency graph and every contributing claim/work request/institution/evidence id, produces a conservative `DERIVED` or `CONJECTURE` composite claim rather than upgrading synthesis to externally verified, bounds confidence by the weakest necessary link with only a small independent-source lift, and records Verification/Audit department events for the composite.
