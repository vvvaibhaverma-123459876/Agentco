...
from scripts.run_pawdent_business_simulation import (
    DECISIONS_PATH,
    FINANCIALS_PATH,
    CALIBRATION_PATH,
    REPORT_PATH,
...
def test_no_sleep_run_produces_36_months_and_source_ids():
    ...
    financial_lines = FINANCIALS_PATH.read_text().splitlines()
...
def test_business_sim_writes_all_required_artifacts():
    _run_fast_simulation()
    for path in [REPORT_PATH, DECISIONS_PATH, FINANCIALS_PATH, CALIBRATION_PATH, SUMMARY_PATH]:
...
def test_false_prediction_never_increases_trust_and_explanation_matches_real_row():
    _run_fast_simulation()
    with CALIBRATION_PATH.open(newline="") as fh:
