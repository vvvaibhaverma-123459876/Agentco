...
BENCH = ROOT / "benchmarks" / "capability_genesis_v5"


def test_genesis_v5_registry_and_hidden_validation_splits():
    registry = json.loads((BENCH / "registry.json").read_text())
    assert registry["benchmark_id"] == "capability-genesis-v5"
...
def test_genesis_v5_software_and_data_rubrics_are_evaluator_owned():
    registry = json.loads((BENCH / "registry.json").read_text())
